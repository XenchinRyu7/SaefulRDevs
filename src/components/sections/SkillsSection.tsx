...
const skillsData = [
  ...
  },
  {
    category: "Databases",
    icon: Database,
    skills: ["PostgreSQL", "MongoDB", "MySQL", "Firebase Firestore", "Redis"]
  },
  {
    category: "DevOps & Tools",
    icon: Zap,
    skills: ["Docker", "Git & GitHub", "CI/CD (Jenkins, GitHub Actions)", "AWS", "Vercel", "Firebase"]
  }
];

export const SkillsSection = () => {
  return (
    <SectionWrapper id="skills" title="My Expertise" icon={Code2} className="bg-secondary/30">
      <div className="grid md:grid-cols-2 gap-8">
        {skillsData.map((categoryItem, index) => (
          <SkillCard key={categoryItem.category} categoryItem={categoryItem} index={index} />
        ))}
      </div>
    </SectionWrapper>
  );
...
const SkillCard = ({ categoryItem, index }: { categoryItem: typeof skillsData[0], index: number }) => {
    ...
    <div {...animProps} className={animProps.className} style={animProps.style}>
      <Card className="h-full shadow-lg hover:shadow-xl transition-shadow duration-300 border-transparent hover:border-primary/30">
        <CardHeader className="flex flex-row items-center space-x-4 pb-4">
          ...
          <CardTitle className="text-2xl font-headline">{categoryItem.category}</CardTitle>
        </CardHeader>
        <CardContent>
